...
from learn_invariance.dataset_3d import R2N2RenderingsTorch
...
class R2N2Renderings(DataSet):
    ...
    def __init__(self, data_path, **kwargs):
        ...
        ds_kwargs = {
            ...
            'custom_class': R2N2RenderingsTorch,
            'label_mapping': None,
            'transform_train': transform, #da.TRAIN_TRANSFORMS_DEFAULT(32),
            'transform_test': transform, #da.TEST_TRANSFORMS_DEFAULT(32)
        }
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
        super(R2N2Renderings, self).__init__('r2n2', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('R2N2(CIFAR) does not support pytorch_pretrained=True')
        return cifar_models.__dict__[arch](num_classes=self.num_classes)

class CIFAR100(DataSet):
    """
    """
    def __init__(self, data_path='/temp', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 100,
