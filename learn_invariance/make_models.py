...
def make_and_restore_model(*_, arch, dataset, resume_path=None,
                           parallel=False, pytorch_pretrained=False,
...
def model_dataset_from_store(s, overwrite_params={}, which='last'):
    ...
    conds = [lambda x: m.schema[x] == s.OBJECT,
             lambda x: m.schema[x] == s.PICKLE]
    ...
    if which == 'last':
        ...
        assert isinstance(which,
                          int), "'which' must be one of {'best', 'last', int}"
