"""
Some functions in this scripts are adapted from https://github.com/MadryLab/robustness
"""
import torch as ch
import numpy as np
...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
    ...
    for p in ['eps', 'attack_lr', 'custom_eps_multiplier']:
        setattr(args, p,
                eval(str(getattr(args, p))) if has_attr(args, p) else None)
    if args.custom_eps_multiplier is not None:
        eps_periods = args.custom_eps_multiplier
        args.custom_eps_multiplier = lambda t: \
            np.interp([t], *zip(*eps_periods))[0]
    ...
    if checkpoint:
        ...
        best_prec1 = checkpoint[prec1_key] if prec1_key in checkpoint \
            ...
            _model_loop(args, 'val', val_loader, model, None, start_epoch - 1,
                        args.adv_train, writer=None)[0]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_prec1, train_loss = _model_loop(args, 'train', train_loader,
                                              model, opt, epoch, args.adv_train,
                                              writer)
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    for i, batch_dict in iterator:

        if args.dataset == 'r2n2':
            if not is_train and args.inv_method == 'reg':
                # to evaluate inv loss, we need to calculate the KL of every two images from an orbit (modify dataset_3d.py to load 24 pre-renderd images)
                samples, targets = batch_dict
                inp = samples[0].cuda()
                target = targets[0].cuda(non_blocking=True)
            else:
                # 0 is fixed while 1 and 2 are randomly picked
                (inp_0, inp_1, inp_2), (target, _, _) = batch_dict
                inp_0, inp_1, inp_2 = inp_0.cuda(), inp_1.cuda(), inp_2.cuda()
                target = target.cuda(non_blocking=True)
                if is_train and args.transform_view and args.inv_method == 'aug':
                    inp = inp_1
                else:
                    inp = inp_0

        else:
            inp, target = batch_dict
            target = target.cuda(non_blocking=True)
            inp = inp.cuda()
        ...
        try:
            ...
            if has_attr(args, "custom_accuracy"):
                ...
                prec1, prec5 = helpers.accuracy(model_logits, target,
                                                topk=(1, maxk))
        ...
        # compute consistency loss if using regularization method
        if args.inv_method == 'reg':
            if is_train:
                output_1, _ = model(inp_1, target=target, make_adv=adv, **attack_kwargs)
                criterion_kl = nn.KLDivLoss(reduction='batchmean', log_target=True)
                reg_term = criterion_kl(F.log_softmax(output), F.log_softmax(output_1))
                losses_reg.update(reg_term.item(), inp.size(0))
                loss = loss + args.inv_method_beta * reg_term

            else:
                # to evaluate inv loss, we need to calculate the KL of every two images from an orbit
                criterion_kl = nn.KLDivLoss(reduction='mean', log_target=True)
                num = 0
                batch_size = target.size(0)
                reg_term = ch.zeros(batch_size, 1).cuda()
                correct = ch.ones(batch_size, 1).cuda()
                for k in range(len(samples)):
                    inp_1 = samples[k].cuda()
                    for j in range(k + 1, len(samples)):
                        inp_2 = samples[j].cuda()
                        output_1, _ = model(inp_1, target=target, make_adv=adv, **attack_kwargs)
                        output_2, _ = model(inp_2, target=target, make_adv=adv, **attack_kwargs)
                        reg_term += criterion_kl(F.log_softmax(output_1), F.log_softmax(output_2))
                        num += 1
                        _, pred_1 = output_1.topk(1, 1, True, True)
                        _, pred_2 = output_2.topk(1, 1, True, True)
                        correct = ch.min(correct, pred_1.eq(pred_2))

                reg_term = reg_term / num
                reg_term = reg_term.mean()
                agree_term = correct.mean()
                losses_reg.update(reg_term.item(), inp.size(0))
                agree_reg.update(agree_term.item(), inp.size(0))
        ...
        if args.inv_method == 'reg':
            if is_train:
                desc = ('{2} Epoch:{0} | Loss {loss.avg:.4f} | '
                        '{1}1 {top1_acc:.3f} | Loss_reg {loss_reg.avg:.5f} | '
                        'Reg term: {reg:.5f} ||'.format(epoch, prec, loop_msg,
                                                        loss=losses, top1_acc=top1_acc,
                                                        loss_reg=losses_reg,
                                                        reg=reg_term))
            else:
                desc = ('{2} Epoch:{0} | Loss {loss.avg:.4f} | '
                        '{1}1 {top1_acc:.3f} | Loss_reg {loss_reg.avg:.5f} | '
                        'Loss_reg_avg: {reg_avg:.5f} | Agree_avg: {agr_avg:.4f} |'
                        'Reg term: {reg:.5f} | Agree term: {agr} |'.format(epoch, prec, loop_msg,
                                                                       loss=losses, top1_acc=top1_acc,
                                                                       loss_reg=losses_reg,
                                                                       reg=reg_term, reg_avg=losses_reg.avg,
                                                                       agr_avg=agree_reg.avg,
                                                                       agr=agree_term))
        else:
            desc = ('{2} Epoch:{0} | Loss {loss.avg:.4f} | '
                    '{1}1 {top1_acc:.3f} | {1}5 {top5_acc:.3f} || '.format(epoch, prec, loop_msg,
                                                                           loss=losses, top1_acc=top1_acc,
                                                                           top5_acc=top5_acc))
